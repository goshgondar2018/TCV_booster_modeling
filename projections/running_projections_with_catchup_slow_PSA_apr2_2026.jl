...
@everywhere function update_params_psa_burn_all(params_start, psa_sample, archetype, waning_scenario)
    ...
    if waning_scenario == "fast"
        params.VE[1:3] .= fill(psa_sample.VE_under2_slow, 3)  #
        params.VE[4] = psa_sample.VE_2_4_slow
        params.VE[5:17] .= fill(psa_sample.VE_5_plus_slow, 13)  
        params.alpha_v[1:3] .= fill(2/psa_sample.duration_under2_slow, 3)  
        params.alpha_v[4] = 2/psa_sample.duration_2_4_slow
        params.alpha_v[5:17] .= fill(2/psa_sample.duration_5_plus_slow, 13) 
    else 
        params.VE[1:4] .= fill(psa_sample.VE_under5_slow, 4)  #
        params.VE[5:17] .= fill(psa_sample.VE_5_plus_slow, 13)  
        params.alpha_v[1:4] .= fill(1/psa_sample.duration_under5_slow, 4)  
...
@everywhere begin
    ...
    psa_samples = CSV.read("params_psa_full_complete_slow.csv", DataFrame)
...
@everywhere function run_projections_for_all_strategies(params_base, strategy_params, current_S, current_Is, current_Ia, current_C, current_R, iteration)
    ...
    age_stratified_cases_df[!, :iteration] .= iteration  # NEW
...
for archetype in 1:3
    ...
    slow_total_cases_list = [result[1] for result in archetype_results_with_campaign]
    slow_pediatric_cases_list = [result[2] for result in archetype_results_with_campaign]
    slow_adult_cases_list = [result[3] for result in archetype_results_with_campaign]
    slow_incidence_list = [result[4] for result in archetype_results_with_campaign]
    slow_population_list = [result[5] for result in archetype_results_with_campaign]
    slow_total_doses_list = [result[6] for result in archetype_results_with_campaign]
    slow_routine_doses_list = [result[7] for result in archetype_results_with_campaign]
    slow_booster_doses_list = [result[8] for result in archetype_results_with_campaign]
    slow_campaign_doses_list = [result[9] for result in archetype_results_with_campaign]
    slow_age_stratified_cases_list = [result[10] for result in archetype_results_with_campaign]  # NEW

    slow_total_cases_with_campaign = vcat(slow_total_cases_list...)
    slow_pediatric_cases_with_campaign = vcat(slow_pediatric_cases_list...)
    slow_adult_cases_with_campaign = vcat(slow_adult_cases_list...)
    slow_incidence_with_campaign = vcat(slow_incidence_list...)
    slow_population_with_campaign = vcat(slow_population_list...)
    slow_total_doses_with_campaign = vcat(slow_total_doses_list...)
    slow_routine_doses_with_campaign = vcat(slow_routine_doses_list...)
    slow_booster_doses_with_campaign = vcat(slow_booster_doses_list...)
    slow_campaign_doses_with_campaign = vcat(slow_campaign_doses_list...)
    slow_age_stratified_cases_with_campaign = vcat(slow_age_stratified_cases_list...)  # NEW

    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_total_cases_with_campaign_apr2.csv", slow_total_cases_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_pediatric_cases_with_campaign_apr2.csv", slow_pediatric_cases_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_adult_cases_with_campaign_apr2.csv", slow_adult_cases_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_incidence_with_campaign_apr2.csv", slow_incidence_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_population_with_campaign_apr2.csv", slow_population_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_total_doses_with_campaign_apr2.csv", slow_total_doses_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_routine_doses_with_campaign_apr2.csv", slow_routine_doses_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_booster_doses_with_campaign_apr2.csv", slow_booster_doses_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_campaign_doses_with_campaign_apr2.csv", slow_campaign_doses_with_campaign)
    CSV.write("./projections_output_check/archetype$(archetype)_slow_waning_age_stratified_cases_with_campaign_apr2.csv", slow_age_stratified_cases_with_campaign)  # NEW
end
