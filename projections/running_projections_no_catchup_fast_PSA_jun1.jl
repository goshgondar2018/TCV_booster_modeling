...
@everywhere function update_params_psa_burn_all(params_start, psa_sample, archetype, waning_scenario)
    ...
    if waning_scenario == "fast"
        ...
        params.VE[1:4] .= fill(psa_sample.VE_under_5_slow, 4)  #
        params.VE[5:17] .= fill(psa_sample.VE_5_plus_slow, 13)  
        params.alpha_v[1:4] .= fill(1/psa_sample.duration_under_5_slow, 4)  
        params.alpha_v[5:17] .= fill(1/psa_sample.duration_5_plus_slow, 13) 
    end
...
@everywhere function typhoid_model_projections!(du, u, p, t) 
    ...
    # total intended vaccination rate
    ...
    # Demographics - aging in/out and mortality
    ...
    ddoses .= actual_total_r_vaxx_ALL .+ actual_total_b_vaxx_ALL 
...
@everywhere function gen_initial_projections(current_S, current_Is, current_Ia, current_C, current_R)
    ...
    initial = vcat(
        ...
        R_never, R_V, R_V2, R_Vw,                 
        cases_0, doses_0, doses_routine_0, doses_booster_0   
    )
...
@everywhere function calc_inc_projections_annual(sol_df)
    ...
    exclude_cols = vcat(case_cols, doses_cols, doses_routine_cols, doses_booster_cols, "time", 
...
@everywhere function update_params_vaxx_covg(local_params, strategy)
    ...
    routine_9m_no_campaign = Dict(
         :routine => Dict(2 => local_params.pvx_r_9mo[1]),
    )
    routine_15m_no_campaign = Dict(
         :routine => Dict(3 => local_params.pvx_r_15mo[1]),
    )
    routine_2y_no_campaign = Dict(
         :routine => Dict(4 => local_params.pvx_r_2yr[1]),
    )
    routine_5y_no_campaign = Dict(
         :routine => Dict(5 => local_params.pvx_r_5yr[1]),
    )
     
    # routine with boosters
    routine_9m_booster_5y_no_campaign = Dict(
        :routine => Dict(2 => local_params.pvx_r_9mo[1]),
        :booster => Dict(5 => local_params.pvx_r_9mo[1]),
    )
    routine_15m_booster_5y_no_campaign = Dict(
        :routine => Dict(3 => local_params.pvx_r_15mo[1]),
        :booster => Dict(5 => local_params.pvx_r_15mo[1]),
    )
    routine_9m_boosters_5y_10y_no_campaign = Dict(
        :routine => Dict(2 => local_params.pvx_r_9mo[1]),
        :booster => Dict(5 => local_params.pvx_r_9mo[1], 6 => local_params.pvx_r_9mo[1]),
    )

    routine_15m_boosters_5y_10y_no_campaign = Dict(
        :routine => Dict(3 => local_params.pvx_r_15mo[1]),
        :booster => Dict(5 => local_params.pvx_r_15mo[1], 6 => local_params.pvx_r_15mo[1]),
    )
    
    if strategy == "routine at 9 months"
        return create_vaccination_params(local_params, routine_9m_no_campaign)
    elseif strategy == "routine at 15 months"
        return create_vaccination_params(local_params, routine_15m_no_campaign) 
    elseif strategy == "routine at 2 years"
        return create_vaccination_params(local_params, routine_2y_no_campaign) 
    elseif strategy == "routine at 5 years"
        return create_vaccination_params(local_params, routine_5y_no_campaign) 
    elseif strategy == "routine at 9 months, booster at 5 years"
        return create_vaccination_params(local_params, routine_9m_booster_5y_no_campaign) 
    elseif strategy == "routine at 15 months, booster at 5 years"
        return create_vaccination_params(local_params, routine_15m_booster_5y_no_campaign) 
    elseif strategy == "routine at 9 months, boosters at 5 and 10 years"
        return create_vaccination_params(local_params, routine_9m_boosters_5y_10y_no_campaign) 
    elseif strategy == "routine at 15 months, boosters at 5 and 10 years"
        return create_vaccination_params(local_params, routine_15m_boosters_5y_10y_no_campaign) 
    end
end
...
# get base parameters for an archetype
...
@everywhere function get_psa_samples(archetype, sample_id)
    if archetype == 1
...
# create parameters for all vaccination strategies
...
# run and save all scenarios output

for archetype in 1:3
    archetype_results_no_campaign = all_archetype_results_no_campaign[archetype]
    
    fast_total_cases_list = [result[1] for result in archetype_results_no_campaign]
    fast_pediatric_cases_list = [result[2] for result in archetype_results_no_campaign]
    fast_adult_cases_list = [result[3] for result in archetype_results_no_campaign]
    fast_incidence_list = [result[4] for result in archetype_results_no_campaign]
    fast_population_list = [result[5] for result in archetype_results_no_campaign]
    fast_total_doses_list = [result[6] for result in archetype_results_no_campaign]
    fast_routine_doses_list = [result[7] for result in archetype_results_no_campaign]
    fast_booster_doses_list = [result[8] for result in archetype_results_no_campaign]
    fast_age_stratified_cases_list = [result[9] for result in archetype_results_no_campaign]  

    fast_total_cases_no_campaign = vcat(fast_total_cases_list...)
    fast_pediatric_cases_no_campaign = vcat(fast_pediatric_cases_list...)
    fast_adult_cases_no_campaign = vcat(fast_adult_cases_list...)
    fast_incidence_no_campaign = vcat(fast_incidence_list...)
    fast_population_no_campaign = vcat(fast_population_list...)
    fast_total_doses_no_campaign = vcat(fast_total_doses_list...)
    fast_routine_doses_no_campaign = vcat(fast_routine_doses_list...)
    fast_booster_doses_no_campaign = vcat(fast_booster_doses_list...)
    fast_age_stratified_cases_no_campaign = vcat(fast_age_stratified_cases_list...)  

    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_total_cases_no_campaign_jun1.csv", fast_total_cases_no_campaign)
    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_pediatric_cases_no_campaign_jun1.csv", fast_pediatric_cases_no_campaign)
    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_adult_cases_no_campaign_jun1.csv", fast_adult_cases_no_campaign)
    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_incidence_no_campaign_jun1.csv", fast_incidence_no_campaign)
    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_population_no_campaign_jun1.csv", fast_population_no_campaign)
    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_total_doses_no_campaign_jun1.csv", fast_total_doses_no_campaign)
    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_routine_doses_no_campaign_jun1.csv", fast_routine_doses_no_campaign)
    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_booster_doses_no_campaign_jun1.csv", fast_booster_doses_no_campaign)
    CSV.write("./projections_output_jun1/archetype$(archetype)_fast_waning_age_stratified_cases_no_campaign_jun1.csv", fast_age_stratified_cases_no_campaign) 
