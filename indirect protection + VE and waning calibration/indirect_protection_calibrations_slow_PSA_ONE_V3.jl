...
@everywhere function update_params_psa_burn(params_start, psa_sample)
    ...
    params.VE[1:4] .= fill(psa_sample.VE_under5, 4)  #
    params.VE[5:17] .= fill(psa_sample.VE_5_plus, 13)  
    params.alpha_v[1:4] .= fill(1/psa_sample.duration_under5, 4)  
    params.alpha_v[5:17] .= fill(1/psa_sample.duration_5_plus, 13)  
    params.r_v .= 0.0
    params.l .= 0.0
    params.alpha_v_2 .= 10
...
@everywhere function typhoid_model_integrated!(du, u, p, t) 
    ...
    waning_V2_to_Vw = p.alpha_v_2 .* V2
    ...
    dVw .+= recover_Is_Vw_to_Vw .+ recover_Ia_Vw_to_Vw .+ waning_R_V_to_Vw .+ waning_R_V2_to_Vw .+ waning_R_Vw_to_Vw .+ waning_V2_to_Vw 
...
# generate initial population sizes for full typhoid model 
...
@everywhere function calc_inc_control_integrated_VE!(sol_archetype,t_start,t_end)
    ...
    # age category definitions for case columns
    cases_agecat1_cols = [:cases_control_1, :cases_control_2, :cases_control_3, :cases_control_4]
    cases_agecat2_cols = [Symbol("cases_control_$i") for i in 5:17]
    ...
    for i in 1:4
        ...
        # Disease compartments (only unvaccinated portion)
        if Symbol("Is_never_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat1 += sum(sol_archetype[:, Symbol("Is_never_$i")][t] for t in t_start:t_end)
        end
        
        if Symbol("Ia_never_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat1 += sum(sol_archetype[:, Symbol("Ia_never_$i")][t] for t in t_start:t_end)
    ...
    for i in 5:17
        # S compartment (all unvaccinated)
        if Symbol("S_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("S_$i")][t] for t in t_start:t_end)
        end    
        
        # disease compartments (only unvaccinated portion)
        if Symbol("Is_never_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("Is_never_$i")][t] for t in t_start:t_end) 
        end
        
        if Symbol("Ia_never_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("Ia_never_$i")][t] for t in t_start:t_end)
        end
        
        if Symbol("C_never_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("C_never_$i")][t] for t in t_start:t_end)
        end
        
        if Symbol("R_never_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("R_never_$i")][t] for t in t_start:t_end) 
        end
    end
    
    # calculate incidence per 100,000 unvaccinated population
     inc_archetype_agecat1 = (total_cases_archetype_agecat1)/(total_person_time_archetype_agecat1)
    inc_archetype_agecat2 = (total_cases_archetype_agecat2)/(total_person_time_archetype_agecat2)
    
    return inc_archetype_agecat1, inc_archetype_agecat2
end
...
@everywhere function calc_inc_vaxx_integrated_VE!(sol_archetype,t_start,t_end)
    ...
    # age category definitions for case columns
    cases_agecat1_cols = [:cases_vaxx_1, :cases_vaxx_2, :cases_vaxx_3, :cases_vaxx_4]
    cases_agecat2_cols = [Symbol("cases_vaxx_$i") for i in 5:17]
    ...
    for i in 5:17
        if Symbol("V_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("V_$i")][t] for t in t_start:t_end)
        end

        if Symbol("V2_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("V2_$i")][t] for t in t_start:t_end)
        end
        
        if Symbol("Vw_$i") in propertynames(sol_archetype) 
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("Vw_$i")][t] for t in t_start:t_end)
        end
        
        # disease compartments (only unvaccinated portion)
        if Symbol("Is_Vw_$i") in propertynames(sol_archetype)
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("Is_Vw_$i")][t] for t in t_start:t_end)
        end
        
        if Symbol("Ia_V_$i") in propertynames(sol_archetype) && Symbol("Ia_V2_$i") in propertynames(sol_archetype) && Symbol("Ia_Vw_$i") in propertynames(sol_archetype)
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("Ia_V_$i")][t] + sol_archetype[:, Symbol("Ia_V2_$i")][t] + sol_archetype[:, Symbol("Ia_Vw_$i")][t] for t in t_start:t_end)
        end
        
        if Symbol("C_V_$i") in propertynames(sol_archetype) && Symbol("C_V2_$i") in propertynames(sol_archetype) && Symbol("C_Vw_$i") in propertynames(sol_archetype)
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("C_V_$i")][t] + sol_archetype[:, Symbol("C_V2_$i")][t] + sol_archetype[:, Symbol("C_Vw_$i")][t] for t in t_start:t_end)
        end
        
        if Symbol("R_V_$i") in propertynames(sol_archetype) && Symbol("R_V2_$i") in propertynames(sol_archetype) && Symbol("R_Vw_$i") in propertynames(sol_archetype)
            total_person_time_archetype_agecat2 += sum(sol_archetype[:, Symbol("R_V_$i")][t] + sol_archetype[:, Symbol("R_V2_$i")][t] + sol_archetype[:, Symbol("R_Vw_$i")][t] for t in t_start:t_end)
        end
    end
...
@everywhere function incidence_gof_calibration!(x, params, target_inc_control_final, target_VE_final, target_VE_post,
        current_S, current_Is, current_Ia, current_C, current_R) 
    
    # Update parameters
    params.l.= x[1]
    params.r_v.= x[2]
    params.alpha_v[1:4] .= x[3]
    ...
    # Parameter bounds check with early return
    ...
    #T_1yr = round(12*1.75) # For final VE and incidence
    #ts_1yr = range(0, stop=T_1yr, step=1)
    ...
    cal_prob_3y = ODEProblem(typhoid_model_integrated!, initial, (0.0, T_3yr), params)
    cal_sol_3y = solve(cal_prob_3y, Tsit5(), save_everystep=false, tstops=ts_3yr, saveat=ts_3yr)
    cleaned_cal_3y = clean_output_integrated!(cal_sol_3y)

    ## control group incidence rate
    cal_inc_control_1y = calc_inc_control_integrated_VE!(cleaned_cal_3y,1,22)
    cal_inc_control_2y = calc_inc_control_integrated_VE!(cleaned_cal_3y,23,30)

    ## vaccine group incidence rate
    cal_inc_vaxx_1y = calc_inc_vaxx_integrated_VE!(cleaned_cal_3y,1,22)
    cal_inc_vaxx_2y = calc_inc_vaxx_integrated_VE!(cleaned_cal_3y,23,30)
    ...
    # 1. Incidence fit (Poisson log-likelihood)
    ...
    # 2. Final VE fit (weighted squared errors for each VE component)
    weights = [1/4, 1/13] #[1/3, 1/13]
    ...
    # 3. Combine objectives
    w_incidence = 10.0 / target_inc_control_final
    
    gof_VE_terms = gof_VE_final + gof_VE_post 
...
@everywhere begin
    ...
    psa_samples = CSV.read("params_psa_full_slow.csv", DataFrame)
...
results_slow = pmap(1:250) do i # change to 251:500, 501:750, and 751:1000 to obtain all samples 
    ...
    # Extract final state more efficiently
    ...
    target_VE_final=[0.744, 0.837]
    target_VE_post=[0.70, 0.793]

    # optimized calibration with tighter bounds
    lower_bounds = [0.5, 0.5, 0.001, 0.001, 0.75, 0.75]  
    upper_bounds = [1.0, 1.0, 0.004, 0.004, 1.0, 1.0]      
    
result_global = bboptimize(x -> incidence_gof_calibration!(x, local_params, target_inc,
                                                              target_VE_final, target_VE_post, current_S, current_Is, current_Ia, current_C, current_R);
    ...
    local_params.alpha_v[1:4] .= best_candidate[3]
    local_params.alpha_v[5:17] .= best_candidate[4]
    local_params.VE[1:4] .= best_candidate[5]
    ...
    inc_final_vaxx = calc_inc_vaxx_integrated_VE!(cleaned_sol_final,1,22)
    inc_final_control = calc_inc_control_integrated_VE!(cleaned_sol_final,1,22)
    VE_final_calibrated = (inc_final_control .- inc_final_vaxx) ./ inc_final_control
    
    inc_post_vaxx = calc_inc_vaxx_integrated_VE!(cleaned_sol_final,23,30)
    inc_post_control = calc_inc_control_integrated_VE!(cleaned_sol_final,23,30)
    VE_post_calibrated = (inc_post_control .- inc_post_vaxx) ./ inc_post_control
    
    # Return results
    return (
        i = i,
        archetype3 = [best_candidate; indirect_protection_calibrated; VE_final_calibrated[1]; VE_final_calibrated[2]; VE_post_calibrated[1]; VE_post_calibrated[2]]
    )
end

# Create result DataFrames (keep your original output code)
names_list_slow = ["l", "r_v", "alpha_v_agecat1","alpha_v_agecat2", "VE_agecat1", "VE_agecat2", "indirect_protection", "VE_final_agecat1", "VE_final_agecat2", "VE_post_agecat1", "VE_post_agecat2"]
indirect_protection_results_slow = DataFrame(Names = names_list_slow)

# Add columns to result DataFrames from the collected results
for result in results_slow
    i = result.i
    col_name = Symbol("run_$i")
    insertcols!(indirect_protection_results_slow, col_name => result.archetype3)
end

CSV.write("indirect_protection_results_slow.csv", indirect_protection_results_slow)
