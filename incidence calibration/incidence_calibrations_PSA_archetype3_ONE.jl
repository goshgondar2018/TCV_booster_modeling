...
@everywhere function incidence_gof_calibration!(x, params_archetype, archetype,target_inc, type, calib)
    ...
    if (calib==1)
        ...
        return(sum(gof_poisson)) #after NM, we want to know the true likelihoods
    end
end
...
results = pmap(1:250) do i
    ...
    local_params_archetype3 = update_params_psa(params_archetype3, samples_archetype3, i)

    T = 1200 # run calibration/burn-in for 100 years
    ts = range(0, stop=T, step=1)
    
    # Incidence targets and initial parameter guesses
    target_inc_archetype3 = [samples_archetype3.inc_age_cat1[i], samples_archetype3.inc_age_cat2[i], 
    samples_archetype3.inc_age_cat3[i], samples_archetype3.inc_age_cat4[i], samples_archetype3.inc_age_cat5[i]]
        
    initial_betas_3 = [3.74*0.75, 6.09*3, 6.09*2, 3.74*2, mean([2.18, 0.439])*0.8]
    initial_f_archetype3 = [f_estimates.estimate[7],f_estimates.estimate[7],f_estimates.estimate[8],f_estimates.estimate[8],f_estimates.estimate[9]]
    initial_alpha = [1/(19.8*12)] 
    initial_params_archetype3 = [initial_betas_3; initial_f_archetype3; initial_alpha]
    
    # Calibrate
    opt_archetype3 = optimize(
        x -> incidence_gof_calibration!(x, local_params_archetype3, 1, target_inc_archetype3, "poisson", 1), 
        initial_params_archetype3, NelderMead(),
        Optim.Options(show_trace=false, g_tol=0.01, f_tol=0.01, x_tol=0.01, iterations=5000)
    )
    opt_burn_archetype3 = Optim.minimizer(opt_archetype3)
 
    # Compute likelihood and incidence
    opt_like_archetype3 = incidence_gof_calibration!(opt_burn_archetype3, local_params_archetype3, 1, target_inc_archetype3, "poisson", 0) 
    opt_inc_archetype3 = get_calibrated_incidence!(local_params_archetype3, opt_burn_archetype3)
    
    # Return results from this run
    return (
        i = i,
        archetype3 = [opt_burn_archetype3; opt_inc_archetype3; opt_like_archetype3]
    )
end
...
archetype3_results = DataFrame(Names = names_list)

for result in results
    i = result.i
    col_name = Symbol("run_$i")
    insertcols!(archetype3_results, col_name => result.archetype3)
end

# Write results to disk
CSV.write("archetype3_all_runs.csv", archetype3_results)
