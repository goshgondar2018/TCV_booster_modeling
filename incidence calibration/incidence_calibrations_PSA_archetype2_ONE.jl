...
# full typhoid model 
# note vaxx compartments don't matter because in pre-vaccine period
...
# process compartmental pop sizes output following transmission 
...
@everywhere begin
    ...
    samples_archetype2 = samples[samples.archetype.=="High",:]
...
results = pmap(1:250) do i
    println(string("starting ", i))
    
    # Initialize parameters for this run
    local_params_archetype2 = update_params_psa(params_archetype2, samples_archetype2, i)

    T = 1200 # run calibration/burn-in for 100 years
    ts = range(0, stop=T, step=1)
    
    # Incidence targets and initial parameter guesses
    target_inc_archetype2 = [samples_archetype2.inc_age_cat1[i], samples_archetype2.inc_age_cat2[i], 
    samples_archetype2.inc_age_cat3[i], samples_archetype2.inc_age_cat4[i], samples_archetype2.inc_age_cat5[i]]
    
    initial_betas_2 = [3.74, 6.09, 6.09, 3.74, mean([2.18, 0.439])]
    initial_f_archetype2 = [f_estimates.estimate[4],f_estimates.estimate[4],f_estimates.estimate[5],f_estimates.estimate[5],f_estimates.estimate[6]]
    initial_alpha = [1/(19.8*12)] 
    initial_params_archetype2 = [initial_betas_2; initial_f_archetype2; initial_alpha]
    
     
    # Calibrate
    opt_archetype2 = optimize(
        x -> incidence_gof_calibration!(x, local_params_archetype2, 1, target_inc_archetype2, "poisson", 1), 
        initial_params_archetype2, NelderMead(),
        Optim.Options(show_trace=false, g_tol=0.01, f_tol=0.01, x_tol=0.01, iterations=5000)
    )
    opt_burn_archetype2 = Optim.minimizer(opt_archetype2)

    # Compute likelihood and incidence
    opt_like_archetype2 = incidence_gof_calibration!(opt_burn_archetype2, local_params_archetype2, 1, target_inc_archetype2, "poisson", 0) 
    opt_inc_archetype2 = get_calibrated_incidence!(local_params_archetype2, opt_burn_archetype2)
 
    # Return results from this run
    return (
        i = i,
        archetype2 = [opt_burn_archetype2; opt_inc_archetype2; opt_like_archetype2]
    )
end
...
archetype2_results = DataFrame(Names = names_list)

for result in results
    i = result.i
    col_name = Symbol("run_$i")
    insertcols!(archetype2_results, col_name => result.archetype2)
end

# Write results to disk
CSV.write("archetype2_all_runs.csv", archetype2_results)
